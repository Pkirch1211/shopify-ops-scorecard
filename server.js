...
// ── GQL Queries ───────────────────────────────────────────────────────────────
const ORDERS_QUERY = ` query Orders($first: Int!, $after: String, $query: String!) { orders(first: $first, after: $after, query: $query, sortKey: UPDATED_AT, reverse: true) { pageInfo { hasNextPage endCursor } edges { node { id name createdAt updatedAt email shippingAddress { firstName lastName } billingAddress { firstName lastName } lineItems(first: 20) { edges { node { quantity } } } fulfillments(first: 5) { createdAt updatedAt displayStatus trackingInfo(first: 1) { company number url } } } } } }`;

const DRAFT_ORDERS_QUERY = ` query DraftOrders($first: Int!, $after: String, $query: String!) { draftOrders(first: $first, after: $after, query: $query, sortKey: UPDATED_AT) { pageInfo { hasNextPage endCursor } edges { node { id name createdAt completedAt status email tags totalPrice subtotalPrice metafield(namespace: "b2b", key: "ship_date") { value } lineItems(first: 50) { edges { node { title variantTitle sku quantity originalUnitPrice variant { id inventoryItem { id } product { tags } } } } } } } } }`;
...
const SKU_HOLDS_QUERY = ` query HoldOrders($first: Int!, $after: String, $query: String!) { orders(first: $first, after: $after, query: $query, sortKey: CREATED_AT) { pageInfo { hasNextPage endCursor } edges { node { id name createdAt email cancelledAt lineItems(first: 100) { edges { node { sku title variantTitle quantity unfulfilledQuantity } } } } } } }`;

const DTC_STALE_QUERY = ` query StaleFulfillments($first: Int!, $after: String, $query: String!) { orders(first: $first, after: $after, query: $query, sortKey: CREATED_AT, reverse: true) { pageInfo { hasNextPage endCursor } edges { node { id name createdAt cancelledAt email displayFulfillmentStatus totalPriceSet { shopMoney { amount currencyCode } } shippingAddress { name address1 address2 city province zip country } tags fulfillments(first: 10) { id name createdAt updatedAt status displayStatus trackingInfo(first: 5) { company number url } events(first: 50) { edges { node { status happenedAt message } } } fulfillmentLineItems(first: 50) { edges { node { quantity lineItem { sku title } } } } } } } } }`;
...
await db.query(`INSERT INTO orders (id, store, order_number, created_at, updated_at, email, units, fulfillment_hours, delivery_hours, processing_hours, is_flagged, flag_types, tracking_json, customer_name) VALUES ${values.join(",")} ON CONFLICT (id) DO UPDATE SET updated_at = EXCLUDED.updated_at, units = EXCLUDED.units, fulfillment_hours = EXCLUDED.fulfillment_hours, delivery_hours = EXCLUDED.delivery_hours, processing_hours = EXCLUDED.processing_hours, is_flagged = EXCLUDED.is_flagged, flag_types = EXCLUDED.flag_types, tracking_json = EXCLUDED.tracking_json, customer_name = EXCLUDED.customer_name, synced_at = NOW()`, params);
...
await db.query(`INSERT INTO stale_fulfillments (fulfillment_id, order_id, order_name, order_created_at, email, customer_name, shipping_address, order_total, fulfilled_at, display_status, has_tracking, tracking_json, skus, tags, latest_event_json, all_events_json, days_since_fulfilled) VALUES ${values.join(",")} ON CONFLICT (fulfillment_id) DO UPDATE SET display_status = EXCLUDED.display_status, has_tracking = EXCLUDED.has_tracking, tracking_json = EXCLUDED.tracking_json, latest_event_json = EXCLUDED.latest_event_json, all_events_json = EXCLUDED.all_events_json, days_since_fulfilled = EXCLUDED.days_since_fulfilled, tags = EXCLUDED.tags, synced_at = NOW()`, params);
}
...
db.query(`SELECT store, COUNT(*) AS total_orders, SUM(units) AS total_units, AVG(fulfillment_hours) AS avg_fulfillment, AVG(delivery_hours) AS avg_delivery FROM orders WHERE created_at >= $1 AND created_at < $2 GROUP BY store`, [start, end]),
db.query(`SELECT id, store, order_number, customer_name, email, units, created_at, flag_types, fulfillment_hours, delivery_hours, processing_hours, tracking_json FROM orders WHERE created_at >= $1 AND created_at < $2 AND is_flagged = TRUE ORDER BY GREATEST(COALESCE(fulfillment_hours,0), COALESCE(delivery_hours,0)) DESC LIMIT 500`, [start, end]),
...
db.query(`SELECT COUNT(*) AS total_orders, SUM(units) AS total_units, AVG(fulfillment_hours) AS avg_fulfillment, AVG(delivery_hours) AS avg_delivery FROM orders WHERE created_at >= $1 AND created_at < $2 AND LOWER(email) = 'care@lifelines.com'`, [start, end]),
db.query(“SELECT value FROM sync_state WHERE key = ‘last_sync’”),
]);
...
try {
const result = await db.query(`SELECT store, TO_CHAR(DATE_TRUNC('month', created_at), 'YYYY-MM') AS month, AVG(fulfillment_hours) AS avg_fulfillment, AVG(delivery_hours) AS avg_delivery, COUNT(*) AS orders FROM orders WHERE created_at >= NOW() - INTERVAL '9 months' AND fulfillment_hours IS NOT NULL GROUP BY store, DATE_TRUNC('month', created_at) ORDER BY DATE_TRUNC('month', created_at)`);
const byStore = {};
...
const CARE_SKU_QUERY = ` query CareOrders($first: Int!, $after: String, $query: String!) { orders(first: $first, after: $after, query: $query, sortKey: CREATED_AT) { pageInfo { hasNextPage endCursor } edges { node { id name createdAt lineItems(first: 50) { edges { node { sku title quantity } } } } } } }`;
...
try {
const result = await db.query(`SELECT tracking_json, order_number, store, created_at, flag_types, fulfillment_hours, delivery_hours FROM orders WHERE is_flagged = TRUE AND created_at >= DATE_TRUNC('year', NOW()) AND tracking_json IS NOT NULL ORDER BY created_at DESC LIMIT 2000`);
